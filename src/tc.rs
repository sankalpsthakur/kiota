...
use crate::interp::{int_bmod, int_ediv, int_emod_nat, int_pow, num_bigint_gcd};
...
impl<'e> Checker<'e> {
    ...
    pub(crate) fn name_str(&self, n: u32) -> &str {
        self.names
            .get(n as usize)
            .map(|s| s.as_str())
            .filter(|s| !s.is_empty())
    ...
    pub(crate) fn pp_budget(&self, e: &Expr, budget: i32) -> String {
        if budget <= 0 {
            return "…".into();
        }
        match &***e {
    ...
    pub fn check_decl(&self, name: u32, kind: &str) -> R<()> {
            ...
            && self.name_str(name).contains("toGraphviz.go._unary.eq_def")
        {
            for (i, nm) in self.names.iter().enumerate() {
                ...
                if s.ends_with("StateT.bind.match_1")
                    || s == "StateT.bind.match_1"
                    || s == "Bind.bind"
                    || s == "Monad.toBind"
                    || s == "StateT.bind"
                    || s == "StateT.instMonad"
    ...
    pub(crate) fn nat_ctors(&self) -> Option<(u32, u32)> {
        let n = self.nat_ref?;
    ...
    fn const_has_proof_arg_uncached(&self, n: u32) -> bool {
        ...
        if self.type_is_unit_ctor_zero_index_prop(ci.typ())
            ...
            || self
                .peel_to_inductive_head(ci.typ())
                .is_some_and(|(_, _, _, _, ity)| self.sort_codomain_is_prop(ity))
        {
            return true;
        }
        ...
        if let (ExprData::Const(n1, u1), ExprData::Const(n2, u2)) = (&**h1, &**h2) {
                    ...
                    .all(|(x, y)| level::is_def_eq(x, y))
            {
                ...
                if !r && std::env::var_os("KIOTA_TRACE_EQ").is_some() && a1.len() >= 4 {
                    eprintln!("EQCORE fail {} na={}", self.name_str(*n1), a1.len());
                }
                return Ok(r);
            }
            return Ok(false);
    ...
    fn whnf_inner(&self, ctx: &Ctx, e: &Expr) -> R<Expr> {
        ...
        if aborted {
            if Self::is_whnf_core_redex(&r) {
                return decline(format!("WHNF core depth limit: {}", self.pp_budget(&r, 12)));
            }
            return Ok(r);
        }
    ...
    fn whnf_core(&self, ctx: &Ctx, e: &Expr) -> R<Expr> {
        ...
        if depth > CONV_DEPTH {
            ...
            if Self::is_whnf_core_redex(&e) {
                return decline(format!("WHNF core depth limit: {}", self.pp_budget(&e, 12)));
            }
            return Ok(e);
        }
        let k = Self::whnf_cache_key(ctx, &e);
    ...
    fn type_is_packed_structure(&self, typ: &Expr) -> bool {
        let Some((_, ctors, num_indices, num_params, _)) = self.peel_to_inductive_head(typ) else {
            return false;
        };
    ...
    pub(crate) fn reduce_nat_arg(&self, ctx: &Ctx, e: &Expr) -> R<Expr> {
    ...
    fn try_nat_extension(&self, ctx: &Ctx, head: &Expr, args: &[Expr]) -> R<Option<Expr>> {
        ...
        match name {
            ...
            "Int.decLe" | "Int.decLt" if args.len() >= 2 => Ok(None),
            ...
            "Neg.neg" if args.len() >= 3 => {
                ...
                let Some(ineg) = self.find_name("Int.neg") else {
                    return Ok(None);
                };
            ...
            "OfNat.ofNat" if args.len() >= 3 => {
                ...
                if ty_name == "Int" {
                    if let Some(n) = self.closed_nat_value(ctx, &args[1])? {
                        if let Some(ofn) = self.find_name("Int.ofNat") {
    ...
    pub(crate) fn find_name(&self, s: &str) -> Option<u32> {
        self.names
            .iter()
...
mod tests {
    ...
    fn idt_false_elim_true_intro_rejects() {
        ...
        env.insert(
            ...
            ConstantInfo::Def {
                level_params: vec![],
                typ: expr::pi(expr::BinderInfo::Default, true_ty.clone(), true_ty.clone()),
    ...
    fn prop_ctor_apps_convert_by_pi_not_field_tree() {
        ...
        assert!(
            ok,
            "nested Cl.mk spines must convert by Prop-ctor skip / PI"
        );
        assert!(
            nested_delta < 50_000,
    ...
    fn large_prop_regulars_convert_by_pi() {
        ...
        env.insert(
            ...
            ConstantInfo::Def {
                level_params: vec![],
                typ: true_ty,
                value: expr::let_(sort1, pad, expr::app(expr::const_(3, vec![]), intro)),
                hints: ReducibilityHints::Regular(2),
                is_unsafe: false,
            },
        );
    ...
    fn heq_nat_vs_int_numeral_not_defeq() {
        ...
        let int3 = expr::apps(
            expr::const_(2, vec![]),
            &[expr::const_(1, vec![]), n3.clone(), expr::const_(3, vec![])],
        );
        let eq = tc.is_def_eq(&Ctx::new(), &n3, &int3).unwrap();
        assert!(!eq, "Nat literal 3 must not defeq OfNat.ofNat Int 3 inst");
    }
    ...
    fn nat_cast_nat_vs_int_not_defeq() {
        ...
        let eq = tc.is_def_eq(&Ctx::new(), &cast(0), &cast(1)).unwrap();
    ...
    fn proj_wrong_inductive_does_not_iota() {
        ...
        let w = tc.whnf(&Ctx::new(), &expr::proj(3, 0, amk)).expect("WHNF");
        assert!(
    ...
    fn lam_domains_must_defeq() {
        ...
        let f_bool = expr::lam(expr::BinderInfo::Default, expr::const_(1, vec![]), star);
    ...
    fn core_depth_abort_declines_not_stuck_whnf() {
        ...
        env.insert(
            ...
            ConstantInfo::Def {
                level_params: vec![],
                typ: expr::pi(expr::BinderInfo::Default, a_ty.clone(), a_ty.clone()),
    ...
    fn pi_body_beta_decrements_outer_bvars() {
        ...
        let nf = expr::pi(expr::BinderInfo::Default, a, expr::app(f, expr::bvar(1)));
        let eq = tc.is_def_eq(&Ctx::new(), &redex, &nf).unwrap();
        assert!(
            eq,
    ...
    fn bind_match_abbrev_iotas_then_defeq_unpacked_vs_proj() {
        ...
        let names = test_names(&["PSigma", "PSigma.mk", "PSigma.rec", "A", "B", "match_1"]);
        let tc = Checker::new(&env, &names, None, None);
        let mut ctx = Ctx::new();
        ctx.push(psigma_ab);
        ...
        assert!(
            ...
            tc.pp(&tc
                .whnf(&ctx, &m_unpacked)
                .unwrap_or_else(|_| m_unpacked.clone())),
    ...
    fn cases_on_lambda_converts_to_rec_lambda() {
        ...
        let lam_cases = expr::lam(expr::BinderInfo::Default, psigma_ab.clone(), cases_app);
        let lam_rec = expr::lam(expr::BinderInfo::Default, psigma_ab, rec_app);
        let eq = tc
            .is_def_eq(&Ctx::new(), &lam_cases, &lam_rec)
            .unwrap_or(false);
        assert!(
            eq,
    ...
    /// Inductive `Foo.Int` is not Lean `Int`: HAdd/cast must not rewrite to `Int.add`.
    #[test]
    ...
    /// `mydite` is not `dite`/`ite`.
    #[test]
    fn mydite_does_not_get_ite_reduction() {
        let mut env = Environment::default();
        let sort1 = expr::sort(level::succ(level::zero()));
        let sort0 = expr::sort(level::zero());
        axiom_sort(&mut env, 0, sort1); // α
        axiom_sort(&mut env, 1, sort0.clone()); // True
        axiom_sort(&mut env, 2, expr::const_(1, vec![])); // True.intro
        axiom_sort(
            &mut env,
            3,
            expr::pi(
        ...
        axiom_sort(&mut env, 4, expr::const_(0, vec![])); // then branch
        axiom_sort(&mut env, 5, expr::const_(0, vec![])); // else branch
        let dite_ty = expr::pi(
            expr::BinderInfo::Default,
        ...
        axiom_sort(&mut env, 6, dite_ty); // mydite
        let names = test_names(&[
            "α",
            "True",
            "True.intro",
        ...
        let tc = Checker::new(&env, &names, None, None);
        let inst = expr::apps(
            expr::const_(3, vec![level::zero()]),
            &[expr::const_(1, vec![]), expr::const_(2, vec![])],
        );
        let e = expr::apps(
            expr::const_(6, vec![]),
            &[
        ...
        let w = tc.whnf(&Ctx::new(), &e).expect("WHNF");
        let (h, _) = expr::unfold_apps(&w);
        let picked_then = matches!(&**h, ExprData::Const(n, _) if *n == 4);
        assert!(
            !picked_then,
            "mydite must not ite-reduce to the then-branch, got {}",
            tc.pp(&w)
        );
    }
